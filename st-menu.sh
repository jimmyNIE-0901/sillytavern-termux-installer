...
get_st_version() {
  if [ -d "$ST_DIR/.git" ]; then
    ...
    if [ -f "package.json" ]; then
      app_version=$(node -p "require('./package.json').version" 2>/dev/null || echo "未知版本号")
    else
      app_version="未知版本号"
    fi

    branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "未知分支")
    commit=$(git rev-parse --short HEAD 2>/dev/null || echo "未知代码")
    date=$(git log -1 --format="%cd" --date=short 2>/dev/null || echo "未知日期")

    echo "v${app_version} / ${branch} / ${commit} / ${date}"
  else
    echo "尚未安装"
  fi
}
...
show_info() {
  ...
  line
  echo "酒馆安装路径：$ST_DIR"
  echo "酒馆目标分支：$ST_BRANCH"
  echo "当前酒馆版本：$(get_st_version)"
  echo "默认端口：$PORT"
  echo "本机访问：http://127.0.0.1:$PORT"
  echo "菜单脚本：$MENU_PATH"
  echo "菜单来源：$MENU_URL"
  echo "Node.js 内存参数：${NODE_OPTIONS:---max-old-space-size=4096}"
  line
  echo
  echo "常见小问题"
  echo
